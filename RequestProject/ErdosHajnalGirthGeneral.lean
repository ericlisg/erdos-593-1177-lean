...
import RequestProject.ErdosHajnalGirth
...
combinatorial engine behind the irreducible core `Erdos593.E2Core` of E2, on top
of the `n = 1` (triangle-free) base case of `RequestProject.ErdosHajnalGirth`.
...
*wrong*: `RequestProject.Attic.E2OddGirthFalse` (`EHG.oddGirthGeneral_false`) exhibits
...
general `k` *is* proved (`RequestProject.EHGirthChromatic`,
`RequestProject.Attic.E2CoreProof.not_colorableBy_general`).  E2 is carried by the paper
...
fully proved in `RequestProject.ErdosHajnalGirth`; `colorableBy_of_le` above is
