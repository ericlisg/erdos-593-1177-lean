import RequestProject.EHBase
import RequestProject.External
...
proved in `RequestProject.EHBase` as `eh_hasKmm`).
...
theorem Dcl_card_le (H : Hypergraph W) (hts : H.IsTripleSystem) (t : ℕ)
  ...
  · refine' le_trans ( mul_le_mul_left' ( ciSup_le _ ) _ ) _;
    exact #X + ℵ₀;
...
theorem cols_step {V : Type*} (n t q : ℕ) (ht : 1 ≤ t) (hn : 1 ≤ n) (hq : n ^ 2 * t ≤ q)
  ...
  have h_bad_card : ∑ z : Fin q, ∑ x ∈ D, (Finset.univ.filter (fun a => P a x = P a z)).card < (q - D.card) * n * t := by
    rw [ Finset.sum_comm ];
    nlinarith [ Nat.sub_add_cancel ( show 1 ≤ t from ht ), Nat.sub_add_cancel ( show D.card ≤ q from le_trans ( Finset.card_le_univ _ ) ( by norm_num ) ), mul_le_mul_left' hD.le q, mul_le_mul_left' hn q, mul_le_mul_left' ht q, mul_le_mul_left' hD.le t, mul_le_mul_left' hn t, mul_le_mul_left' ht t ];
...
theorem rows_step {V : Type*} (n t q : ℕ) (ht : 1 ≤ t) (hn : 1 ≤ n)
  ...
  have hC : C.card ≤ 2 * n ^ 3 * t := by
    ...
    nlinarith [ mul_le_mul_left' hn t ];
