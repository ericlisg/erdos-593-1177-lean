import RequestProject.AmalgClosure
import RequestProject.DecompReverse
...
* `NegativeCore` — the negative half of #593 and the spectrum construction
  ...
  complete-rank one-apex lift (§3, **fully proved** in `RequestProject.Lift`),
...
fully proved in `RequestProject.AmalgClosure` (elementary apart from the de
Bruijn–Erdős compactness theorem, itself proved in `RequestProject.Compactness`).
-/
...
complete-rank one-apex lift (§3, fully proved in `RequestProject.Lift`), the
...
in `RequestProject.DecompReverse` (the finite bridge decomposition, via the
separation dichotomy `RequestProject.DecompExpansion` — amalgamation at a bridge
...
(`RequestProject.AmalgClosure`), and the expansion case by the carried input
`ReiherExpansion` (E4). -/
