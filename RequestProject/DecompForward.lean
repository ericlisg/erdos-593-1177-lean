import RequestProject.Structures
...
theorem expansion_private {VJ : Type} [Fintype VJ] [DecidableEq VJ] (J : SimpleGraph VJ)
    ...
    (hmem : Sum.inr e ∈ s) :
    s = {Sum.inl (Quot.out e.1).1, Sum.inl (Quot.out e.1).2, Sum.inr e} := by
  convert expansion_edge_cases J hs;
  grind
...
theorem expansion_linear {VJ : Type} [Fintype VJ] [DecidableEq VJ] (J : SimpleGraph VJ)
  ...
  have h_eq : a₁.val = s(p, q) ∧ a₂.val = s(p, q) := by
    grind +suggestions;
  grind +splitImp
...
theorem expansion_even {VJ : Type} [Fintype VJ] [DecidableEq VJ] (J : SimpleGraph VJ)
  ...
  have hadj : ∀ i, J.Adj (a i) (a (i + 1)) := by
    ...
    exact h_adj;
  convert zmod_even_of_alt ( fun i => ( col ( a i ) : ZMod 2 ) ) _;
  · exact ⟨ by linarith [ c.hm ] ⟩;
  · intro i; specialize hadj i; specialize hcol hadj; simp_all +decide [ SimpleGraph.completeGraph ] ;
    grind
...
theorem disjUnion_berge_side {F G : FTS} (c : BergeCycle (F.disjUnion G)) :
  ...
  have h_const : ∀ i : ZMod c.m, s i = s 0 := by
    have hconst : ∀ i, s i = s (i + 1) := by
      intro i;
      have := c.mem_left i; have := c.mem_right i; simp_all +decide [ FTS.disjUnion ] ;
      grind +suggestions;
...
theorem disjUnion_bridge {F G : FTS} (hF : F.IntrinsicObligatory)
  ...
  · obtain ⟨ w, hw₁, hw₂ ⟩ := hG.2.1 ⟨ d, hd ⟩;
    ...
    · refine' ⟨ ⟨ w, hw₁, rfl ⟩, _ ⟩;
      ...
      · specialize hw₂ ( BergeCycle.ofDisjRight c hv ( disjUnion_berge_edges_right c hv ) ) i ; simp_all +decide [ BergeCycle.ofDisjRight ];
        grind +suggestions
