import RequestProject.DecompDichotomy
...
theorem walk_split_at {V : Type*} {G : SimpleGraph V} {u v : V} (w : G.Walk u v)
  ...
  induction' w with u v w ih generalizing k;
  · cases k <;> simp +decide at hk ⊢;
  · cases k <;> simp_all +decide [ SimpleGraph.Walk.length_cons ];
    obtain ⟨ w1, ⟨ w2, hw ⟩, hw' ⟩ := ‹∀ k ≤ _, ∃ w1, ( ∃ x, _ ) ∧ _› _ hk; use SimpleGraph.Walk.cons ‹_› w1; aesop;
...
def coreGraph (F : FTS) (pr : {e : Finset F.V // e ∈ F.edges} → F.V) :
  ...
  symm := by rintro a b ⟨h1, ed, h2, h3⟩; exact ⟨h1.symm, ed, h3, h2⟩
...
theorem F_iso_expansion (F : FTS) (pr : {e : Finset F.V // e ∈ F.edges} → F.V)
  ...
  have key : ∀ (ed : {e : Finset F.V // e ∈ F.edges})
    ...
    have hmkout : Sym2.mk ((Quot.out x.1).1, (Quot.out x.1).2) = x.1 := by
      rw [Prod.mk.eta]; exact Quot.out_eq x.1
