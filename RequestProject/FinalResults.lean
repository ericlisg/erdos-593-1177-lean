import RequestProject.Consequences
import RequestProject.ReiherPassage
...
machinery has been fully proved (`RequestProject.AmalgClosure`,
`RequestProject.DecompReverse`, `RequestProject.Lift`, `RequestProject.Reservoir`,
`RequestProject.NegativeDirection`, `RequestProject.CycleCollapse`,
`RequestProject.Calibration`, …), and every remaining carried input has been
reduced to the **five literature theorems E1–E5** stated in
`RequestProject.External`.
