...
Corollary 9.7) is now a **proved theorem** (`Erdos593.e3_EGH_P`, in
`RequestProject.E3Proof`), obtained from the universal level-graph construction
of Erdős–Galvin–Hajnal Theorem 8.1 (which is what actually yields property `P`).

Combined with the already-discharged **E4** (`Erdos593.e4_Reiher`,
`RequestProject.E4Proof`), every headline result that used to carry E3 and E4 as
