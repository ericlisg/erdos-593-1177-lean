import RequestProject.DeltaRho
...
theorem deltaRho_regular (hρ : ℵ₀ ≤ ρ) : (deltaRho ρ).ord.cof = deltaRho ρ := by
  ...
  have hIioLt : ∀ s : δ.ord.ToType, #(Set.Iio s) < δ := by
    intro s
    rw [show #(Set.Iio s) = #{y : δ.ord.ToType // y < s} from rfl, Ordinal.card_typein s]
    exact (Cardinal.lt_ord).mp (Ordinal.typein_lt_self (o := δ.ord) s)
  ...
  obtain ⟨S, hSub, hScard⟩ := Ordinal.cof_eq (α := δ.ord.ToType) (· < ·)
  rw [show Ordinal.type (α := δ.ord.ToType) (· < ·) = δ.ord from Ordinal.type_toType δ.ord] at hScard
