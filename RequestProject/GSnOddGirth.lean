...
import RequestProject.GSnPotential
...
`RequestProject.GSn` contains **no odd cycle of length `≤ 2n+1`**
...
`≤ n²+n`.  The max-window potential `pot` of `RequestProject.GSnPotential` then
